...
public class DimensionalPattern {
    ...
    public static BlockState createBlockState(char type, Block block) {
        return new BlockState(type, block, 0, true);
    }

    public static BlockState createBlockState(char type, Block block, int meta) {
        return new BlockState(type, block, meta, true);
    }
    ...
    public static class BlockState implements IPatternComponent {
        ...
        public BlockState(char type, Block block, int meta, boolean checkMeta) {
            this.type = type;
            this.block = block;
            this.meta = meta;
        ...
        public boolean isMatched(IBlockAccess world, int x, int y, int z) {
        	if(checkMeta) return world.getBlock(x, y, z).equals(block) && world.getBlockMetadata(x, y, z) == meta;
        	else return world.getBlock(x, y, z).equals(block);
        }
    }
